...
class Auth implements DBA {
  ...
  Future<String> signInWithGoogle() async {
    ...
    try {
      await this.googleAuth().then((value) async {
        DocumentSnapshot userSnap = await this
            .getUserSnapShot(value.user.uid); // check user registered or not
  ...
  Future<bool> sendPushNotification(String token, String title, String body,
      {var data, var tokens}) async {
  ...
  Future<String> signUp(String email, String password) async {
    try {
      User u = (await dbAuth.createUserWithEmailAndPassword(
  ...
  Future<void> updateUserClout(String userID, int newClout) {
    ...
    dbRef.collection(db_users_collection).doc(userID).get().then((value) {
      ...
      dbRef
          .collection(db_users_collection)
          .doc(userID)
          .update({'userVibe.clout': newClout.toString()});
    });
  }
  ...
  Future<void> updateUserThemePreferences(String userID, String val) async {
    dbRef
        .collection(db_users_collection)
        .doc(userID)
        .update({'userVibe.lituationPrefs': val});
  }
  ...
  Future<void> updateUserPreference(String userID, String val) async {
    dbRef
        .collection(db_users_collection)
        .doc(userID)
        .update({'userVibe.preference': val});
  }
  ...
  Future<void> updateUserLocation(
      String userID, String val, LatLng point) async {
    dbRef
        .collection(db_users_collection)
        .doc(userID)
        .update({'userLocation': val}).then((value) {
  ...
  Future<void> setVibeVisibility(String userID, String val) async {
    dbRef
        .collection(db_user_settings_collection)
        .doc(userID)
        .update({'vibe_visibility': val});
  }
  ...
  Future<void> setLituationVisibility(String userID, String val) async {
    dbRef
        .collection(db_user_settings_collection)
        .doc(userID)
        .update({'lituatiom_visibility': val});
  }
  ...
  Future<void> setActivityVisibility(String userID, String val) async {
    dbRef
        .collection(db_user_settings_collection)
        .doc(userID)
        .update({'activity_visibility': val});
  }
  ...
  Future<void> setLocationVisibility(String userID, String val) async {
    dbRef
        .collection(db_user_settings_collection)
        .doc(userID)
        .update({'location_visibility': val});
  }
  ...
  Future<void> setLituationNotifications(String userID, bool val) async {
    dbRef
        .collection(db_user_settings_collection)
        .doc(userID)
        .update({'lituation_notifications': val});
  }
  ...
  Future<void> setInvitationNotifications(String userID, bool val) async {
    dbRef
        .collection(db_user_settings_collection)
        .doc(userID)
        .update({'invitation_notifications': val});
  }
  ...
  Future<void> setGeneralNotifications(String userID, bool val) async {
    dbRef
        .collection(db_user_settings_collection)
        .doc(userID)
        .update({'general_notifications': val});
  }
  ...
  Future<void> updateUserBio(String userID, String val) async {
    dbRef
        .collection(db_users_collection)
        .doc(userID)
        .update({'userVibe.bio': val});
  }

  Future<void> updateUserAttendancePreference(
      String userID, String newAttendancePreference) async {
    dbRef
        .collection("users")
        .doc(userID)
        .update({'userVibe.preference': newAttendancePreference});
  }

  Future<void> sendInvite(m.Invitation invite) async {
    await dbRef.collection("users_lituations").doc(invite.recipient).update({
      "invitations": FieldValue.arrayUnion([invite.invitationID])
    });
    await dbRef.collection("users_lituations").doc(invite.senderID).update({
      "invitations": FieldValue.arrayUnion([invite.invitationID])
    });
    await dbRef.collection("lituations").doc(invite.lituationID).update({
      "invited": FieldValue.arrayUnion([invite.recipient])
    });
  }

  Future<void> acceptInvitation(m.Invitation invite) async {
    await dbRef.collection("users_lituations").doc(invite.recipient).update({
      "invitations": FieldValue.arrayRemove([invite.invitationID])
    });
    await dbRef.collection("users_lituations").doc(invite.senderID).update({
      "invitations": FieldValue.arrayRemove([invite.invitationID])
    });
    await dbRef.collection("lituations").doc(invite.lituationID).update({
      "invited": FieldValue.arrayRemove([invite.recipient])
    });
    await dbRef.collection("lituations").doc(invite.lituationID).update({
      "vibes": FieldValue.arrayUnion([invite.recipient])
    });
...
* */
    ...
    dbRef.collection('vibing').doc(visited).get().then((value) {
      ...
      if (!pending.contains(visitor) && !vibes.contains(visitor)) {
        dbRef
            .collection('vibing')
            .doc(visited)
            .update({'pendingVibing': FieldValue.arrayUnion(vo)}).then((value) {
          dbRef
              .collection('vibed')
              .doc(visitor)
              .update({"pendingVibes": FieldValue.arrayUnion(vd)});
        });
        sendPushNotification(vistedObj.deviceToken, "Vibe Request",
            "${visitorObj.username} has requested to vibe with you");
      } else {
        if (pending.contains(visitor)) {
          dbRef.collection('vibing').doc(visited).update(
              {'pendingVibing': FieldValue.arrayRemove(vo)}).then((value) {
            dbRef
                .collection('vibed')
                .doc(visitor)
                .update({"pendingVibes": FieldValue.arrayRemove(vd)});
          });
          sendPushNotification(vistedObj.deviceToken, "Vibe Request Approve",
              "${visitorObj.username} vibe with you");
        }
        if (vibes.contains(visitor)) {
          dbRef
              .collection('vibing')
              .doc(visited)
              .update({'vibing': FieldValue.arrayRemove(vo)}).then((value) {
            dbRef
                .collection('vibed')
                .doc(visitor)
                .update({'vibed': FieldValue.arrayRemove(vd)});
            updateClout(visited, -3);
          });
          sendPushNotification(vistedObj.deviceToken, "Vibe Remove",
              "${visitorObj.username} remove vibe with you");
        }
      }
    });
  }
  ...
  Future<void> addToPendingVibes(String user, String vibeID) async {
    dbRef.collection("vibed").doc(user).update({
      'pendingVibes': FieldValue.arrayUnion([vibeID])
    }).then((value) {
      dbRef.collection("vibing").doc(vibeID).update({
        'pendingVibing': FieldValue.arrayUnion([user])
      });
      return;
    });
  }
  ...
  Future<void> removePendingVibe(String user, String vibeID) async {
    dbRef.collection("vibed").doc(user).update({
      'pendingVibes': FieldValue.arrayRemove([vibeID])
    }).then((value) {
      dbRef.collection("vibing").doc(vibeID).update({
        'pendingVibing': FieldValue.arrayRemove([user])
      });
      return;
    });
  }
  ...
  Future<void> updateClout(String userID, int cloutAmt) async {
    int newClout;
    await dbRef.collection("users").doc(userID).get().then((val) {
  ...
  Future<void> updateLituationEndDate(String lID, DateTime newEndDate) async {
    await dbRef
        .collection("lituations")
        .doc(lID)
        .update({'end_date': Timestamp.fromDate(newEndDate)});
  }
  ...
  Future<void> updateLituationLocationLatLng(
      String lID, LatLng newLocationLatLng) async {
    await dbRef.collection("lituations").doc(lID).update({
      'locationLatLng':
          GeoPoint(newLocationLatLng.latitude, newLocationLatLng.longitude)
    });
  }
  ...
  Future<void> addLikeLituation(String userId, String lID) async {
    ...
    await dbRef
        .collection('lituations')
        .doc(lID)
        .update({"likes": FieldValue.arrayUnion(data)});
    await dbRef
        .collection('lituations')
        .doc(lID)
        .update({"clout": FieldValue.increment(5)});
  }
  ...
  Future<void> addDislikeLituation(String userId, String lID) async {
    ...
    await dbRef
        .collection('lituations')
        .doc(lID)
        .update({"dislikes": FieldValue.arrayUnion(data)});
    await dbRef
        .collection('lituations')
        .doc(lID)
  ...
  Future<void> removeLikeLituation(String userId, String lID) async {
    ...
    await dbRef
        .collection('lituations')
        .doc(lID)
        .update({"likes": FieldValue.arrayRemove(data)});
  }
  ...
  Future<void> removeDislikeLituation(String userId, String lID) async {
    ...
    await dbRef
        .collection('lituations')
        .doc(lID)
        .update({"dislikes": FieldValue.arrayRemove(data)});
  }
    ...
    dbRef.collection('lituations').doc(lID).get().then((value) async {
      ...
      if (!lit.observers.contains(userID)) {
        dbRef
            .collection('lituations')
            .doc(lID)
            .update({'observers': FieldValue.arrayUnion(u)}).then((value) {
          dbRef
              .collection('users_lituations')
              .doc(userID)
              .update({"observedLituations": FieldValue.arrayUnion(l)});
        });
        if (hoster.deviceToken != "")
          sendPushNotification(hoster.deviceToken, "New Overserver",
              "${visitor.username} has is observing your ${lit.title}");
      } else {
        dbRef
            .collection('lituations')
            .doc(lID)
            .update({'observers': FieldValue.arrayRemove(u)}).then((value) {
          dbRef
              .collection('users_lituations')
              .doc(userID)
              .update({"observedLituations": FieldValue.arrayRemove(l)});
        });
        if (hoster.deviceToken != "")
          sendPushNotification(hoster.deviceToken, "Quit Overserver",
              "${visitor.username} has quitted your ${lit.title}");
      }
    });
  }
    ...
    dbRef.collection('lituations').doc(lID).get().then((value) {
      ...
      if (lit.pending.contains(userID)) {
        dbRef
            .collection('lituations')
            .doc(lID)
            .update({'pending': FieldValue.arrayRemove(u)}).then((value) {
          dbRef
              .collection('lituations')
              .doc(lID)
              .update({'vibes': FieldValue.arrayUnion(u)}).then((value) {
            dbRef.collection('users_lituations').doc(userID).update(
                {'pendingLituations': FieldValue.arrayRemove(l)}).then((value) {
              dbRef.collection('users_lituations').doc(userID).update({
                'upcomingLituations': FieldValue.arrayUnion(l)
              }).then((value) async {
                ...
                if (user.deviceToken != "")
                  sendPushNotification(
                      user.deviceToken,
                      "Approved for lituation",
                      "You have been approved for ${lit.title}");
              });
            });
          });
        });
    ...
    dbRef.collection('lituations').doc(lID).get().then((value) {
      ...
      if (lit.vibes.contains(userID)) {
        dbRef
            .collection('lituations')
            .doc(lID)
            .update({'vibes': FieldValue.arrayRemove(u)}).then((value) {
          dbRef.collection('users_lituations').doc(userID).update(
              {'pendingLituations': FieldValue.arrayRemove(l)}).then((value) {
            dbRef
                .collection('users_lituations')
                .doc(userID)
                .update({'upcomingLituations': FieldValue.arrayRemove(l)}).then(
                    (value) async {
              //TODO send approval message
              UserModel.User user = await getUserModel(userID);
              if (user.deviceToken != "")
                sendPushNotification(user.deviceToken, "Removed for lituation",
                    "You have been removed for ${lit.title}");
            });
          });
        });
      }
    ...
    dbRef.collection('lituations').doc(lID).get().then((value) async {
      ...
      if (!lit.pending.contains(userID)) {
        dbRef
            .collection('lituations')
            .doc(lID)
            .update({'pending': FieldValue.arrayUnion(u)}).then((value) {
          dbRef.collection('users_lituations').doc(userID).update(
              {"pendingLituations": FieldValue.arrayUnion(l)}).then((value) {
            if (hoster.deviceToken != "")
              sendPushNotification(hoster.deviceToken, "RSVP",
                  "${user.username} has rsvp'd for your ${lit.title}");
          });
        });
      } else {
        dbRef
          ...
          dbRef.collection('users_lituations').doc(userID).update(
            ...
            if (hoster.deviceToken != "")
              sendPushNotification(hoster.deviceToken, "RSVP",
                  "${user.username} withdraw rsvp'd for your ${lit.title}");
          });
        });
      }
    });
    ...
    dbRef.collection('lituations').doc(lID).get().then((value) async {
      ...
      if (!lit.vibes.contains(userID)) {
        dbRef
            .collection('lituations')
            .doc(lID)
            .update({"vibes": FieldValue.arrayUnion(n)}).then((value) {
          dbRef.collection('users_lituations').doc(userID).update(
              {"upcomingLituations": FieldValue.arrayUnion(l)}).then((value) {
            if (hoster.deviceToken != "")
              sendPushNotification(hoster.deviceToken, "New guest",
                  "New User on the guest list");
          });
        });
      } else {
        dbRef
            .collection('lituations')
            .doc(lID)
            .update({"vibes": FieldValue.arrayRemove(n)}).then((value) {
          dbRef.collection('users_lituations').doc(userID).update(
              {"upcomingLituations": FieldValue.arrayRemove(l)}).then((value) {
            if (hoster.deviceToken != "")
              sendPushNotification(hoster.deviceToken, "Remove guest",
                  "User removed from the guest list");
          });
        });
      }
    });
  ...
  Future<void> removeInvitedUser(String userId, String lID) async {
    ...
    dbRef
        .collection('lituations')
        .doc(userId)
        .update({"invited": FieldValue.arrayRemove((data))});
  }
  ...
  Future<void> removeDraft(String userId, String lID) async {
    ...
    dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"drafts": FieldValue.arrayRemove(data)});
  }
  ...
  Future<void> removeUserLituation(String userId, String lID) async {
    ...
    dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"lituations": FieldValue.arrayRemove((data))});
  }
  ...
  Future<void> removePastLituation(String userId, String lID) async {
    ...
    dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"pastLituations": FieldValue.arrayRemove((data))});
  }
  ...
  Future<void> removeUpcomingLituation(String userId, String lID) async {
    ...
    dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"upcomingLituations": FieldValue.arrayRemove((data))});
  }
  ...
  Future<void> removeWatchedLituation(String userId, String lID) async {
    ...
    dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"observedLituations": FieldValue.arrayRemove((data))});
  }
  ...
  Future<void> removePendingLituation(String userId, String lID) async {
    ...
    dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"pendingLituations": FieldValue.arrayRemove((data))});
  }

  Future<void> removeInvitationLituation(
    ...
    await dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"invitations": FieldValue.arrayRemove(data)});
    await dbRef
        .collection('lituations')
        .doc(lID)
        .update({"invited": FieldValue.arrayRemove(invites)});
  }
  ...
  Future<List> uploadLituationMedia(
      Lituation l, List imgs, UserModel.User hoster) async {
    ...
    sendPushNotification(
      hoster.deviceToken,
      "Media upload",
      "Your media for ${l.title} have been succesfully uploaded!",
    );
    return urls;
  }
  ...
  Future<TaskSnapshot> upload(File imageFile, Lituation l) async {
    ...
    TaskSnapshot uploadTask = await dbMediaRef
        .ref()
        .child('lituationThumbnails')
        .child(l.hostID)
        .child('lituations')
        .child(l.eventID)
        .child(filename)
      ...
      await value.ref.getDownloadURL().then((url) {
        dbRef.collection('lituations').doc(l.eventID).update({
          "thumbnail": FieldValue.arrayUnion([url.toString()])
        });
      });
      return value;
    });
  ...
  Future<void> addToUserDrafts(String userId, String lID) async {
    ...
    dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"drafts": FieldValue.arrayUnion(data)});
  }
  ...
  Future<void> addToUserLituations(String userId, String lID) async {
    ...
    dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"lituations": FieldValue.arrayUnion(data)});
  }
  ...
  Future<void> addToUpcomingLituations(String userId, String lID) async {
    ...
    dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"upcomingLituations": FieldValue.arrayUnion(data)});
  }
  ...
  Future<void> addToUserInvitation(
    ...
    await dbRef
        .collection('users_lituations')
        .doc(userId)
        .update({"invitations": FieldValue.arrayUnion(data)});
    await dbRef
        .collection('lituations')
        .doc(lID)
        .update({"invited": FieldValue.arrayUnion(invites)});
  }
...
//VIBE Queries
  Future<void> acceptPendingVibe(String userID, String vibeID) async {
    ...
    cancelPendingVibeRequest(vibeID, userID).then((value) => {
          dbRef
              .collection('vibing')
              .doc(userID)
              .update({'vibing': FieldValue.arrayUnion(u)}).then((value) async {
            dbRef
                .collection('vibed')
                .doc(vibeID)
                .update({'vibed': FieldValue.arrayUnion(m)});
            ...
            sendPushNotification(
              vibe.deviceToken,
              "Accept Vibe",
              "${user.username} has accepted your vibe request",
            );
          })
        });
  }
      ...
      if (lit.pending.contains(userID)) {
        dbRef
          ...
          dbRef
              .collection('users_lituations')
              .doc(userID)
              .update({'pendingLituations': FieldValue.arrayRemove(l)}).then(
                  (value) async {
            ...
            sendPushNotification(
              user.deviceToken,
              "Deny RSVP",
              "Your RSVP to ${lit.title} was denided.",
            );
          });
        });
      }
  ...
  Future<void> removeVibed(String visitor, String visited) async {
    ...
    dbRef
        .collection('vibed')
        .doc(visitor)
        .update({"vibed": FieldValue.arrayRemove(v)}).then((value) {
      dbRef
          .collection('vibing')
          .doc(visitor)
          .update({"vibing": FieldValue.arrayRemove(u)}).then((value) {
        return;
      });
    });
  }
  ...
  Future<void> removeInvited(String id) async {
    dbRef
        .collection('lituations')
        .doc(getLituationIdFromInvitation(id))
        .update({
      "invited": FieldValue.arrayRemove([getRecipientIdFromInvitation(id)])
    }).then((value) {
      dbRef
          .collection('users_lituations')
          .doc(getRecipientIdFromInvitation(id))
          .update({
  ...
  Future<void> removeVibing(String visitor, String visited) async {
    ...
    dbRef
        .collection('vibing')
        .doc(visitor)
        .update({"vibing": FieldValue.arrayRemove(v)}).then((value) {
      dbRef
          .collection('vibed')
          .doc(visitor)
          .update({"vibed": FieldValue.arrayRemove(u)}).then((value) {
        return;
      });
    });
  }
            ...
            dbRef.collection('users_lituations').doc(userID).update(
              ...
              dbRef.collection('users_lituations').doc(userID).update({
                ...
                sendPushNotification(
                  user.deviceToken,
                  "Approve RSVP",
                  "You have approved for and added to the guest list for ${lit.title}",
                );
              });
            });
          });
  ...
  Future<void> cancelPendingVibeRequest(String userID, String vibeID) async {
    ...
    dbRef
        .collection('vibing')
        .doc(userID)
        .update({"pendingVibing": FieldValue.arrayRemove(u)}).then((value) {
      dbRef
          .collection('vibed')
          .doc(vibeID)
          .update({"pendingVibes": FieldValue.arrayRemove(m)});
      dbRef
          .collection('vibed')
          .doc(userID)
          .update({"pendingVibes": FieldValue.arrayRemove(u)}).then((value) {
        dbRef
            .collection('vibing')
            .doc(vibeID)
            .update({"pendingVibing": FieldValue.arrayRemove(m)});
      });
    });
  }
  ...
  Future<void> completeRegistration(UserModel.User u) async {
    ...
    await dbRef.collection('users').doc(id).set(u.toJson()).then((value) {
      dbRef
          .collection(db_vibed_collection)
          .doc(id)
          .set(initNewUserVibed(id).toJson())
          .then((value) {
        dbRef
            .collection(db_vibing_collection)
            .doc(id)
            .set(initNewUserVibing(id).toJson())
            .then((value) {
          dbRef
              .collection(db_user_lituations_collection)
              .doc(id)
              .set(initNewUserLituations(id).toJson())
              .then((value) {
            dbRef
                .collection(db_user_settings_collection)
                .doc(id)
                .set(initNewUserSettings(id).toJson())
                .then((value) {
              dbRef
                  .collection(db_user_activity_collection)
                  .doc(id)
                  .set(initNewUserActivity(id).toJson())
                  .then((value) {
                return;
              });
            });
  ...
  Future<void> resetSettingToDefault(String userID) {
    dbRef
        .collection(db_user_settings_collection)
        .doc(userID)
        .set(initNewUserSettings(userID).toJson());
  }
  ...
  createChatRoom(ChatRoomModel chatroom) async {
    ...
    for (int i = 1; i < chatroom.party.length; i++) {
      ...
      if (chatroom.party.length == 2) {
        sendPushNotification(p.deviceToken, "Chat",
            "${creator.username} starting a chat with you");
      } else {
        sendPushNotification(p.deviceToken, "Chat",
            "You have been added to ${chatroom.room_name}");
      }
    }
  }
  ...
  sendMessageToRoom(String roomID, ChatMessage m) async {
    ...
    for (int i = 0; i < chatroom.party.length; i++) {
      String element = chatroom.party[i];
      if (m.user.uid == element) continue;
      UserModel.User p = await getUserModel(element);
      sendPushNotification(
          p.deviceToken,
          "Chat",
  ...
  getUserChatRooms(String userID) async {
    return await dbRef
        .collection('chat')
        .where("party", arrayContains: userID)
        .get();
  }
  ...
  Future<QuerySnapshot> getUserInvitations(String userID, String lID) async {
    return await dbRef
        .collection('users_lituations')
        .where("invitations", arrayContains: lID + ":" + userID)
        .get();
  }
  ...
  Stream<DocumentSnapshot> getChatRoomParty(String roomID) {
    return dbRef
        ...
        .doc(roomID)
        .snapshots()
        .where((event) => event.data().containsKey('party'));
  }
  ...
  void setUserRateofLituation(
      String litID, String to, String from, int rate) async {
    await dbRef.collection('lituations').doc(litID).update({
      'rates': FieldValue.arrayUnion([
        {
          'to': to,
          'from': from,
          'rate': rate, //true: thumb up, false: thumb down,
