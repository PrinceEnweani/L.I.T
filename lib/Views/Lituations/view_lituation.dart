...
class _ViewLituationState extends State<ViewLituation> {
  ...
  Widget lituationDetailsProvider(AsyncSnapshot l) {
    ...
    return Scaffold(
        ...
        appBar: topNav(
            backButton(),
            pageTitle(lit.title, Theme.of(context).textSelectionColor),
            [shareButton()],
              ...
              children: [
                ListView(
                  ...
                  children: <Widget>[
                    ...
                    lituationInfoCard(
                        "Entry",
                        '${lit.entry}${lit.entry == 'Fee' ? ' (\$' + lit.fee + ')' : ''}',
  ...
  Widget ratingRow(
      BuildContext context, List<String> likes, List<String> dislikes) {
        ...
        buttons: [
          DialogButton(
            child: Text(
              ...
              style: TextStyle(
                  color:
                      Theme.of(_scaffoldKey.currentContext).textSelectionColor,
                  fontSize: 18),
            ),
            onPressed: () {
              Navigator.pop(_scaffoldKey.currentContext);
  ...
  Widget ticketLituationButton(Lituation lit) {
    ...
    if (isPaid == false && isHoster == false) return Container();
    return Container(
        height: 50,
            ...
            child: Row(
              ...
              children: [
                Expanded(
                  flex: 7,
                  child: Text(
                    isHoster ? "Check Ticket" : "Show Ticket",
                    style: infoValue(Theme.of(context).textSelectionColor),
              ...
              if (isHoster) {
                ...
                showDialog(
                    ...
                    builder: (BuildContext context) {
                      return QRViewer(
                          lit: lit, userID: widget.lituationVisit.userID);
                    });
              }
            },
  ...
  Widget bottomButtonsProvider(Lituation l) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [invitedNotifier(l), bottomButtons(l)],
      ),
    );
  }
    ...
    return StreamBuilder(
      ...
      builder: (context, u) {
        ...
        return Container(
          ...
          child: RichText(
              text: TextSpan(
                  text: parseVibes('You were invited by '),
                  style: infoValue(Theme.of(context).textSelectionColor),
                  children: [
                TextSpan(
  ...
  onPaymentFinish(res, Lituation _lit) async {
    if (res == true) {
      await showDialog(
          context: context, builder: (_) => PaymentSuccessDialog());
      await lp.attendLituation();
      User _user = await lp.db.getUserModel(widget.lituationVisit.userID);
      String res = await sendTicketEmail(
          "Ticket for ${_lit.title}", _user.email, _user.userID, _lit);
      if (res == "Sended") {
        _scaffoldKey.currentState.showSnackBar(
            sBar('A ticket has been sent to your email.', c: Colors.green));
      } else {
        _scaffoldKey.currentState.showSnackBar(sBar(
            'An error while sending ticket to your email.',
            c: Colors.redAccent));
      }
      setState(() {
        isLoading = false;
  ...
  Widget attendButton(bool going, bool rsvpd, String entry, Lituation _lit) {
    ...
    if (entry.toLowerCase().contains(INVITE_ONLY) ||
        entry.toLowerCase().contains(PRIVATE_ENTRY)) {
      String val = 'RSVP';
      Color col = Colors.green;
      if (rsvpd) {
        col = Colors.red;
        ...
        onPressed: isLoading
            ...
            : () {
                if (going == false && entry == "Fee") {
                  ...
                  PaymentService.pay(int.parse(_lit.fee ?? "1"), _lit, context,
                      onPaymentFinish);
                } else {
                  Alert(
                    context: context,
  ...
  Widget hostBottomButtons(Lituation l) {
    if (l.status != LIT_PENDING) {
      return Container(
          child: Text("The lituation is ${lituationStatus(l.status)}"));
    }
    return Container(
  ...
  Widget lituationCapacityProvider(AsyncSnapshot l) {
    ...
    return lituationInfoCard(
        'Capacity', capacityChecker(l.data['capacity']), Icons.person);
  }
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: Text(
                    '(' +
                        addedObservers.length.toString() +
                        ') vibes are observing',
                    style: infoLabel(Theme.of(context).primaryColor),
                  ),
                ),
                Container(
                  ...
                  child: ListView.builder(
                      ...
                      itemBuilder: (context, idx) {
                        return circularProfileWidget(
                            observers[idx].data()['profileURL'],
                            observers[idx].data()['userID'],
                            observers[idx].data()['username'],
                            observers[idx].data()['status']['status']);
                      }),
                ),
              ],
            ),
  ...
  Future<void> _updateLituation(Lituation l, Lituation update) async {
    ...
    if (update.locationLatLng != null &&
        update.locationLatLng != l.locationLatLng) {
  ...
  Future<void> initLituationLocation(AsyncSnapshot l) async {
    ...
    locationIcon =
        await getBytesFromAssetFile('assets/images/litlocationicon.png', 250);
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            margin: EdgeInsets.fromLTRB(0, 10, 0, 0),
                            child: Text(
                              updatedLituation.location != null
                                  ? updatedLituation.location
                                  : value,
                              style: TextStyle(
                                  color: Theme.of(context).textSelectionColor),
                              textAlign: TextAlign.start,
    ...
    return Container(
      ...
      child: GoogleMap(
        ...
        initialCameraPosition:
            CameraPosition(target: lituationLatLng, zoom: zoom),
      ),
    );
  }
    ...
    return Container(
      ...
      child: TypeAheadField(
            ...
            decoration: InputDecoration(
                ...
                enabledBorder: UnderlineInputBorder(
                  borderSide: BorderSide(
                      color: Theme.of(context).textSelectionColor, width: 1),
                ),
        ...
        suggestionsCallback: (pattern) async {
          var locationIcon = await getBytesFromAssetFile(
              'assets/images/litlocationicon.png', 225);
            ...
            if (value.length > 0) {
              ...
              for (PlacesSearchResult place in places) {
                ...
                resultMarkers.add(googleMapMarker(
                    place.name, BitmapDescriptor.fromBytes(locationIcon), pos));
              }
  ...
  Widget showCap(AsyncSnapshot l) {
    return Container(
      ...
      child: Text(
        'Change Capacity:' + "\n" + capacityChecker(l.data['capacity']),
        style: infoValue(Theme.of(context).textSelectionColor),
        textAlign: TextAlign.left,
      ),
    );
  ...
  Widget capacityPicker(
      AsyncSnapshot l, IconData descIcon, String title, String value) {
    return Card(
              ...
              children: <Widget>[
                ...
                new Container(
                    ...
                    child: new Text(title,
                        style:
                            infoValue(Theme.of(context).textSelectionColor))),
                new Expanded(
                  //width: 175,
  ...
  Widget capacityInput() {
    bool hasCapacity;
    if (updatedLituation.capacity != null &&
        updatedLituation.capacity.toLowerCase().contains('max')) {
      hasCapacity = true;
    } else {
      hasCapacity = false;
    }
      ...
      return Container(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
              ...
              labelText: 'Cap: ' + updatedLituation.capacity != null
                  ? updatedLituation.capacity
                  : 'is 0',
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: Text(
                    addedIDs.length == 1
                        ? '(' + addedIDs.length.toString() + ') is going'
                        : '(' + addedIDs.length.toString() + ') are going',
                    style: infoLabel(Theme.of(context).primaryColor),
                  ),
                ),
                Row(
  ...
  Widget lituationTimeProvider(AsyncSnapshot l) {
    ...
    return Column(
      children: [
        lituationInfoCard(
            "Date",
            DateFormat.yMEd().format(dateTimeToTimeStamp(l.data['date'])),
            MaterialCommunityIcons.calendar),
        lituationInfoCard(
            "Time:",
            parseDateToEndDate(l.data['date'], l.data['end_date']),
            Icons.access_time),
      ],
    );
  }
  ...
  Widget editableStartEndTimeWidget(AsyncSnapshot l) {
    ...
    return Column(
      children: [
        datePicker(Icons.calendar_today_outlined, 'Date',
            showDate('New Date', str), editableDatePicker(l)),
        datePicker(
            Icons.access_time,
            'Start Time',
            showDate('New Start time', parseTime(l.data['date'])),
            cupertinoStartTimePicker(l)),
        datePicker(
            Icons.cancel,
            'End Time',
            showDate('New End time', parseTime(l.data['end_date'])),
            cupertinoEndTimePicker(l)),
      ],
    );
  }
    ...
    return Container(
      ...
      child: CupertinoTheme(
        ...
        child: CupertinoDatePicker(
          minimumDate:
              dateTimeToTimeStamp(l.data['date']).add(new Duration(days: 1)),
          mode: CupertinoDatePickerMode.date,
          initialDateTime:
              dateTimeToTimeStamp(l.data['date']).add(new Duration(days: 2)),
        ...
        child: CupertinoDatePicker(
          ...
          onDateTimeChanged: (DateTime c) {
            setState(() {
              ...
              DateTime d = DateTime(
                  _date.year, _date.month, _date.day, c.hour, c.minute);
  ...
  Widget cupertinoEndTimePicker(AsyncSnapshot l) {
    ...
    return Container(
      ...
      child: CupertinoTheme(
        ...
        child: CupertinoDatePicker(
          minimumDate:
              tommorow().add(new Duration(hours: 1, minutes: 0, seconds: 0)),
          mode: CupertinoDatePickerMode.time,
  ...
  Widget datePicker(
      IconData descIcon, String title, Widget display, Widget picker) {
    return Card(
      margin: EdgeInsets.all(10),
      elevation: 10,
      color: Theme.of(context).scaffoldBackgroundColor,
            ...
            child: Column(
              children: [
                Container(
                  ...
                  margin: EdgeInsets.fromLTRB(15, 5, 0, 5),
                  child: Text(
                    'attending vibes (' + attendees.length.toString() + ')',
                    style:
                        infoLabelMedium(Theme.of(context).textSelectionColor),
                  ),
                ),
                Container(
                  margin: EdgeInsets.fromLTRB(0, 15, 15, 0),
  ...
  Widget cancelableCircularProfileWidget(
      String url, String userID, String username, String status) {
    bool online = getStatusAsBool(status);
    return GestureDetector(
      onTap: () {
  ...
  Widget lituationHostInfoProvider(String hostID) {
    return StreamBuilder(
      ...
      builder: (ctx, u) {
        if (u.hasData == false || u.connectionState != ConnectionState.done)
          return new Text(
            "loading",
  ...
  Widget hostInfo(
      String hostID, String username, String url, BuildContext ctx) {
        ...
        child: Column(
          children: [
            ...
            Container(
                ...
                padding: EdgeInsets.fromLTRB(0, 0, 0, 0),
                child: Text('Themes:',
                    style: infoLabelMedium(Theme.of(context).primaryColor))),
            Container(
                margin: EdgeInsets.fromLTRB(15, 0, 0, 0),
                alignment: Alignment.centerLeft,
                padding: EdgeInsets.fromLTRB(0, 0, 0, 0),
                child: Text(parseThemesFromSnapShot(l),
                    style: infoValueMedium(Theme.of(context).indicatorColor))),
          ],
        ),
      ),
    );
  ...
  Widget editableLituationTitleWidget(l) {
    if (tec.text == '') {
      tec = new TextEditingController(text: l.data['title']);
    }
    if (tec2.text == '') {
      tec2 = new TextEditingController(text: l.data['description']);
    }
    return Card(
      elevation: 3,
      color: Theme.of(context).scaffoldBackgroundColor,
  ...
  void showGuestList() {
    Navigator.pushNamed(context, GuestListPageRoute,
        arguments: widget.lituationVisit);
  }
  ...
  Widget pendingCircularProfileWidget(
      String url, String userID, String username, String status) {
    bool online = getStatusAsBool(status);
    return GestureDetector(
      onTap: () {
  ...
  Widget shareButton() {
    return Container(
      ...
      child: IconButton(
        icon: Icon(
          popUpMenu ? Icons.cancel : Icons.share,
          color: Theme.of(context).buttonColor,
          size: 25,
        ),
        onPressed: () {
